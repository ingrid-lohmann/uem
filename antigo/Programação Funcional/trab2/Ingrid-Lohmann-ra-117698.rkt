...
(define (esta-na-lista lista valor)
 (cond
    [(empty? lista) 0]
    [(= (first lista) valor) (add1 0)]
    [else (esta-na-lista (rest lista) valor)])
)


;;Confere a quantidade de alternativas corretas por questão, retorna 0 caso o candidato tenha marcado alguma alternativa errada
;; Lista(Números) -> Número 
(examples
  (check-equal? (confere-numero-corretas '(1 2 3) '(1 2 3)) 3)
  (check-equal? (confere-numero-corretas '(2 3 4 5 6) '(1 7 8 9)) 0)
  (check-equal? (confere-numero-corretas '(21 10 08 16 15) '(4 10 4 16 10)) 0)
)


(define (confere-numero-corretas gabarito resposta)
  (define corretas (map 
    (lambda (n) (esta-na-lista gabarito n))
    resposta))
  (cond
    [(= (esta-na-lista corretas 0) 1) 0]
    [else
      (soma-elementos
        (map 
...
(examples
  ...
  (check-equal? (pontuacao-por-questao 12 5) 0)
...
(examples
  (check-equal? (pontuacao-parcial-respostas '(23 7 0 12 18) '(16 5 0 5 2)) 14.5)
  (check-equal? (pontuacao-parcial-respostas '(23 10 8 16 15) '(4 10 4 16 10)) 16.5)
  (check-equal? (pontuacao-parcial-respostas '(23 7 31 12 18) '(10 4 8 15 9)) 3.2)
...
(define (pontuacao-parcial-respostas gabarito respostas)
  (soma-elementos
    (map 
      (lambda (x y) (pontuacao-por-questao x y))
      gabarito
      respostas))
)
...
(examples
  (check-equal? (pontuacao-final '(23 7 0 12 18) 333 '(16 5 0 5 2) 80) '(333 94.5))
  (check-equal? (pontuacao-final '(23 10 8 16 15) 231 '(4 10 4 16 10) 60) '(231 76.5))
  (check-equal? (pontuacao-final '(23 7 31 12 18) 987 '(10 4 8 15 9) 0) '(987 3.2))
...
(define (filtra-classificados candidatos-lista)
  (filter-not empty? 
    (map 
        (lambda (lista) (if (= (second lista) 0) '() lista))
        candidatos-lista))
)
...
(examples
  ...
  (check-equal? (classificacao-final (list 21 10 8 16 15) '((345 5 (23 7 31 12 18)))) '((345 5)))
...
(define (classificacao-final gabarito candidatos-lista)
  ...
  (map 
     (lambda (lista) (pontuacao-final gabarito (first lista) (third lista) (second lista)))
     lista-final)
)
